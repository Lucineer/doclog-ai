...
## Identity
- **Vessel**: Doclog Ai
- **Role**: AI-powered Doclog companion
- **URL**: https://doclog-ai.casey-digennaro.workers.dev
- **Repo**: github.com/Lucineer/doclog-ai
- **Branch**: master
...
### Deploy
```bash
cd /tmp/doclog-ai && wrangler deploy
```

### Health Check
```bash
curl -s https://doclog-ai.casey-digennaro.workers.dev/health
curl -s https://doclog-ai.casey-digennaro.workers.dev/vessel.json | python3 -m json.tool
```
...
- **Vessel Tuner**: https://vessel-tuner.casey-digennaro.workers.dev/api/vessel?name=doclog-ai
...
- **vessel.json capabilities**: document-management, knowledge-base
...
1. Keep the vessel small. ~164 lines is the current size.
